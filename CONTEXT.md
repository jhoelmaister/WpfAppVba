...
App WPF separada (`VisorEmpresa/VisorEmpresa.csproj`, símbolo de compilación `VISOR`) que visualiza la **empresa completa** (todas las sucursales a la vez), a diferencia de la app principal que siempre trabaja filtrada por `AppState.SucursalActiva`. Login propio (`LoginVisorWindow`), ventana propia (`ConsolaVisor.xaml` — define su propia clase `namespace WpfAppVba { class ConsolaMovimientos }` con la misma API de pestañas que la app principal, truco que permite que los archivos vinculados no necesiten saber en qué app corren), tema propio (`TemaVisor`).
...
### Patrón de archivos: vinculados vs. copia propia
`VisorEmpresa.csproj` reutiliza gran parte del código de `WpfAppVba` vía **archivos vinculados** (`<Compile Include="..\WpfAppVba\Xxx.cs" Link="..."/>`) — una sola fuente de verdad, sin copias, para todo lo que no necesita comportamiento distinto entre apps:
- **Infraestructura genérica** (sin lógica de negocio): `DatabaseConnection`, `ConexionConfig`/`ConexionEstado`, `PasswordHasher`, `WindowTheming`/`WindowHelper`, `GridFocusHelper`, `FuncionesComunes`, `SqlRetry`.
- **Capa de datos compartida**: `SqlData`/`DataConsulta` (motor de caché genérico), `AppLoader` (`ConectarProductos` puebla catálogos ya a nivel de empresa — `Sql.ArticulosObj`, `Sql.DocumentosLObj`/`PreciosObj`, `Sql.RegionesObj`, etc. — sin filtro de sucursal), `AppState` (estado de sesión compartido — ver advertencia abajo), `StockCalculator`.
- **Módulos de catálogo puro sin divergencia**: `PreciosGeneral`, `EmpresasGeneral`/`Detalle`, `SucursalesGeneral`/`Detalle`, `UsuariosGeneral`/`Detalle`, `RegionesGeneral`/`Detalle` — funcionan igual en ambas apps, sin necesidad de copia propia.

Tienen **copia física propia** en `VisorEmpresa/` (namespace `VisorEmpresa`, NO vinculados) los archivos con comportamiento genuinamente distinto entre apps:
- `PedidosGeneral`/`TraspasosGeneral`/`CorreccionesGeneral`/`FacturasGeneral`: filtran por combo de sucursal (`_sucursalFiltro`, "Todas las sucursales" o una puntual) + año (`VisorState.AnioActivo`) en vez de `AppState.SucursalActiva`; sin botones de gestión.
- `PedidosDetalle`/`TraspasosDetalle`/`CorreccionesDetalle`/`FacturasDetalle`/`PreciosDetalle`: siempre de solo lectura (excepto Precios, que sí guarda) — ver sesión 2026-07-08 para el detalle de por qué se separaron de la versión vinculada.

### `ConsultasEmpresa.cs` — consultas propias del visor
Consultas SQL agregadas a nivel de EMPRESA (todas las sucursales), con conexión propia (no la global de `AppLoader`, para poder correr en paralelo sin choque de `SqlConnection`/`DataReader`). Incluye `ConectarCache{Pedidos,Traspasos,Correcciones,Facturas}` (puebla las mismas cachés `Sql.DocumentosXObj`/`XObj` que usan los `*General`/`*Detalle`, pero con SQL propio scoped a una sucursal o toda la empresa) y `ObtenerStockEmpresa`/`ObtenerStockEmpresaAlCierre` (stock/disponible por artículo a nivel de empresa, cacheado en memoria — ver sesión 2026-07-08).

### ⚠️ Trampa recurrente: campos de `AppState` que nunca se pueblan en el visor
`AppState.SucursalActiva`, `AppState.AperturaActiva`, `AppState.RegionActiva`, `AppState.DataFechaInicio`/`DataFechaFinal` **nunca se completan en VisorEmpresa** — los llena `AppState.ActualizarBase()`, exclusivo del flujo de login de la app principal (`LoginVisorWindow` deja `SucursalActiva`/`RegionActiva` explícitamente en `""`). Cualquier código vinculado/reusado que dependa de estos campos (p. ej. `StockCalculator.ContarStock`, que usa los tres primeros) da resultados vacíos o en cero cuando corre dentro del visor, sin ningún error visible — ya causó dos bugs reales en la sesión 2026-07-08. Antes de reusar un método de la app principal dentro de un archivo propio de VisorEmpresa, revisar si depende de alguno de estos campos; si es así, hay que reemplazarlo por el equivalente de `ConsultasEmpresa`/`VisorState` (que sí tiene `EmpresaActiva`/`AnioActivo`/`UsuarioActivo`, sin sucursal ni apertura activa — el visor es multi-sucursal por diseño).
