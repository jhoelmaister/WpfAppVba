...
{
    /// <summary>
    /// Vista de CORRECCIONES de stock de toda la empresa, solo visualización:
    /// filtro por sucursal (todas o una) y por año, con corte por la apertura de
    /// cada sucursal — ver ConsultasEmpresa.CargarDocsCorrecciones.
    /// </summary>
    public partial class CorreccionesVisor : UserControl
    {
        ...
        private async Task CargarDatosAsync()
        {
            ...
            {
                Mouse.OverrideCursor = Cursors.Wait;
                _docs = await Task.Run(() => ConsultasEmpresa.CargarDocsCorrecciones(emp, anio, suc));
        ...
        private async void Grid1_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            try
            {
                var lineas = await Task.Run(() => ConsultasEmpresa.CargarLineasCorreccion(fila.Id));
