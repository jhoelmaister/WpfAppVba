...
namespace VisorEmpresa
{
    ...
    public partial class InformePdfArticulos : Window
    {
        ...
        // SistemaGestion.InformePdfArticulos / InventariosGeneral.GenerarPdfInventario
        // (banda celeste de grupo, encabezado gris, tabla de totales por categoría
        // alineada al borde derecho de la columna Stock, fila "Total general"
        // destacada). Stock de TODA la empresa (ConsultasEmpresa), no por sucursal. ──
        private void GenerarPdf(string filePath, DateTime fechaCorte)
        {
            ...
            for (int i = 1; i <= uf; i++)
            {
                ...
                if (!CumpleCondicion(totales.Stock, condicion)) continue;
